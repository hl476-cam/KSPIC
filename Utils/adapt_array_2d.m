function [recon,cmap]=adapt_array_2d(yn,data,rn,norm)
...
%-------------------------------------------------------------------------
%	Input:
%	yn:  array data for calculating weights [ny, nx, nc] (bright-blood data)
%   data: array data to be combined [ny, nx, nc](subtracted data).
...
if rem(nx,st)~=0       % HL added on 06/09/2018
    st=2;
end
...
recon=squeeze(sum(wfull.*data));   %Combine coil signals. 
